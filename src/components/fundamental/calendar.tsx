...
function Calendar(calendarProps: CalendarProps) {
  ...
  const dayPickerProps = {
    ...
    classNames: {
      ...
      button_previous: cn(
        "pointer-events-auto",
        "inline-flex items-center justify-center rounded-[var(--border-radius-m)]",
        ...
        "hover:bg-[var(--color-background-neutral-subtlest-hovered)] hover:border-[var(--color-border-action-outline-hovered)] hover:shadow-[var(--shadow-200)]",
      ...
      weekdays: "flex",
      weekday: "text-[var(--color-text-tertiary)] rounded-[var(--border-radius-m)] w-[var(--size-m)] font-normal text-caption-medium-sm",
      ...
      selected: "!bg-[var(--color-background-blue-bold)] [&_button]:!text-white [&_button]:after:!bg-white rounded-[var(--border-radius-m)]",
    ...
    components: {
      Chevron: ({ orientation, ...props }: any) => {
        const LucideIcon = orientation === "left" ? ChevronLeft : ChevronRight;
        const { size, ...iconProps } = props;
        return <Icon name={LucideIcon} size="s" {...iconProps} />;
      },
    },
    ...props,
  } as any;
