...
const FileUploadRoot = React.forwardRef<HTMLDivElement, FileUploadProps>(
  ({
    ...
    const contextValue: FileUploadContextValue = React.useMemo(
      ...
      [files, onFilesChange, maxFiles, maxSize, accept, disabled, multiple, onUpload]
    )
  ...
  "relative flex flex-col items-center justify-center rounded-lg border-2 border-dashed p-6 text-center transition-colors duration-200",
  {
    variants: {
      isDragActive: {
        ...
        false: "border-[var(--color-border-input)] hover:border-[var(--color-border-brand)]",
      },
      disabled: {
...
const FileUploadDropzone = React.forwardRef<HTMLDivElement, FileUploadDropzoneProps>(
  ({ className, children, ...props }, ref) => {
    const { files, onFilesChange, maxFiles, maxSize, accept, disabled, multiple } = useFileUpload()
    ...
    return (
      <div
        ...
        className={cn(
          ...
          file.status === "error" && "border-[var(--color-border-error)] bg-[var(--color-background-error-subtle)]",
          file.status === "success" && "border-[var(--color-border-success)] bg-[var(--color-background-success-subtle)]",
          className
        )}
        {...props}
      >
...
const FileUploadItemPreview = React.forwardRef<HTMLDivElement, FileUploadItemPreviewProps>(
  ({ className, file, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("flex h-10 w-10 items-center justify-center rounded", className)}
        {...props}
      >
        {file.preview ? (
          <img
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2">
          <p className="text-body-sm font-medium truncate">{file.file.name}</p>
          <Badge
            variant={
              file.status === "success" ? "default" :
              file.status === "error" ? "secondary" :
              file.status === "uploading" ? "secondary" : "secondary"
            }
          >
            {file.status}
          </Badge>
